...
namespace GeneXus.Programs {
   public class reorg : GXReorganization
   {
      ...
      public void CreateTripAttraction( )
      {
         string cmdBuffer = "";
         /* Indices for table TripAttraction */
         try
         {
            cmdBuffer=" CREATE TABLE [TripAttraction] ([TripId] smallint NOT NULL , [AttractionId] smallint NOT NULL , PRIMARY KEY([TripId], [AttractionId]))  "
            ;
         ...
         catch
         {
            try
            {
               DropTableConstraints( "[TripAttraction]") ;
               cmdBuffer=" DROP TABLE [TripAttraction] "
               ;
               RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
               RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
               RGZ.ExecuteStmt() ;
            ...
            catch
            {
               ...
               {
                  DropTableConstraints( "[TripAttraction]") ;
                  cmdBuffer=" DROP VIEW [TripAttraction] "
                  ;
                  RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
                  RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
                  RGZ.ExecuteStmt() ;
               ...
               catch
               {
                  ...
                  {
                     DropTableConstraints( "[TripAttraction]") ;
                     cmdBuffer=" DROP FUNCTION [TripAttraction] "
                     ;
                     RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
         ...
         try
         {
            cmdBuffer=" CREATE NONCLUSTERED INDEX [ITRIPATTRACTION1] ON [TripAttraction] ([AttractionId] ) "
            ;
            RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
            RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
            RGZ.ExecuteStmt() ;
         ...
         {
            cmdBuffer=" DROP INDEX [ITRIPATTRACTION1] ON [TripAttraction] "
            ;
            RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
            RGZ.ErrorMask = GxErrorMask.GX_MASKNOTFOUND | GxErrorMask.GX_MASKLOOPLOCK;
            RGZ.ExecuteStmt() ;
      ...
      {
         string cmdBuffer = "";
         /* Indices for table Trip */
         try
         {
            cmdBuffer=" CREATE TABLE [Trip] ([TripId] smallint NOT NULL IDENTITY(1,1), [TripDate] datetime NOT NULL , [TripDescription] nvarchar(200) NOT NULL , PRIMARY KEY([TripId]))  "
            ;
            RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
            RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
            RGZ.ExecuteStmt() ;
         ...
         catch
         {
            try
            {
               DropTableConstraints( "[Trip]") ;
               cmdBuffer=" DROP TABLE [Trip] "
               ;
               RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
               RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
               RGZ.ExecuteStmt() ;
      ...
      public void RITripAttractionTrip( )
      {
         string cmdBuffer;
         try
         {
            cmdBuffer=" ALTER TABLE [TripAttraction] ADD CONSTRAINT [ITRIPATTRACTION2] FOREIGN KEY ([TripId]) REFERENCES [Trip] ([TripId]) "
            ;
            RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
            RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
            RGZ.ExecuteStmt() ;
         ...
         catch
         {
            try
            {
               cmdBuffer=" ALTER TABLE [TripAttraction] DROP CONSTRAINT [ITRIPATTRACTION2] "
               ;
               RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
               RGZ.ErrorMask = GxErrorMask.GX_MASKNOTFOUND | GxErrorMask.GX_MASKLOOPLOCK;
               RGZ.ExecuteStmt() ;
      ...
      public void RITripAttractionAttractions( )
      {
         string cmdBuffer;
         try
         {
            cmdBuffer=" ALTER TABLE [TripAttraction] ADD CONSTRAINT [ITRIPATTRACTION1] FOREIGN KEY ([AttractionId]) REFERENCES [Attractions] ([AttractionId]) "
            ;
            RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
            RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
            RGZ.ExecuteStmt() ;
         ...
         catch
         {
            try
            {
               cmdBuffer=" ALTER TABLE [TripAttraction] DROP CONSTRAINT [ITRIPATTRACTION1] "
               ;
               RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
               RGZ.ErrorMask = GxErrorMask.GX_MASKNOTFOUND | GxErrorMask.GX_MASKLOOPLOCK;
               RGZ.ExecuteStmt() ;
      ...
      private bool PreviousCheck( )
      {
         ...
         if ( tableexist("TripAttraction",sSchemaVar) )
         {
            SetCheckError ( GXResourceManager.GetMessage("GXM_table_exist", new   object[]  {"TripAttraction"}) ) ;
            return false ;
         }
         if ( tableexist("Trip",sSchemaVar) )
         {
            SetCheckError ( GXResourceManager.GetMessage("GXM_table_exist", new   object[]  {"Trip"}) ) ;
            return false ;
         }
         return true ;
      }
      ...
      {
         ReorgExecute.RegisterBlockForSubmit( 1 ,  "CreateTripAttraction" , new Object[]{ });
         ReorgExecute.RegisterBlockForSubmit( 2 ,  "CreateTrip" , new Object[]{ });
      }

      private void ExecuteOnlyRisReorganization( )
      {
         ReorgExecute.RegisterBlockForSubmit( 3 ,  "RITripAttractionTrip" , new Object[]{ });
         ReorgExecute.RegisterBlockForSubmit( 4 ,  "RITripAttractionAttractions" , new Object[]{ });
      }
      ...
      {
         GXReorganization.SetMsg( 1 ,  GXResourceManager.GetMessage("GXM_filecrea", new   object[]  {"TripAttraction", ""}) );
         ReorgExecute.RegisterPrecedence( "CreateTripAttraction" ,  "CreateTrip" );
         GXReorganization.SetMsg( 2 ,  GXResourceManager.GetMessage("GXM_filecrea", new   object[]  {"Trip", ""}) );
      }

      private void SetPrecedenceris( )
      {
         GXReorganization.SetMsg( 3 ,  GXResourceManager.GetMessage("GXM_refintcrea", new   object[]  {"[ITRIPATTRACTION2]"}) );
         ReorgExecute.RegisterPrecedence( "RITripAttractionTrip" ,  "CreateTripAttraction" );
         ReorgExecute.RegisterPrecedence( "RITripAttractionTrip" ,  "CreateTrip" );
         GXReorganization.SetMsg( 4 ,  GXResourceManager.GetMessage("GXM_refintcrea", new   object[]  {"[ITRIPATTRACTION1]"}) );
         ReorgExecute.RegisterPrecedence( "RITripAttractionAttractions" ,  "CreateTripAttraction" );
      }
